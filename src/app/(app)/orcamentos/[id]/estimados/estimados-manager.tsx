...
export function EstimadosManager({ orcamentoId, arvore, totalGeral }: { orcamentoId: string; arvore: CadernoNode[]; totalGeral: number }) {
  ...
  const estadoInicial = useMemo(() => {
    ...
    for (const { node } of linhas) {
      ...
      m.set(node.id, { estimado: node.estimado || sugerido, motivo: node.estimado_motivo ?? '' })
    }
    return m
  }, [linhas])
  ...
  const houveMudanca = useMemo(() => {
    for (const { node } of linhas) {
      ...
      const salvo = { estimado: node.estimado, motivo: node.estimado_motivo ?? '' }
      if (!atual) continue
  ...
  // soma só os nós marcados que não têm ancestral também marcado.
  const totalEstimado = useMemo(() => {
    ...
    function percorrer(nodes: CadernoNode[], ancestralMarcado: boolean) {
      for (const n of nodes) {
        ...
        if (marcadoAqui && !ancestralMarcado) soma += n.total
  ...
  async function salvar() {
    ...
    for (const { node } of linhas) {
      ...
      if (!atual) continue
      const salvo = { estimado: node.estimado, motivo: node.estimado_motivo ?? '' }
      if (atual.estimado !== salvo.estimado || (atual.estimado && atual.motivo.trim() !== salvo.motivo.trim())) {
        alteracoes.push({ id: node.id, estimado: atual.estimado, motivo: atual.motivo.trim() || null })
      }
    }
    try {
    ...
    <div className="space-y-4">
      <div className="rounded-lg border border-amber-200 bg-amber-50/60 p-4">
        <p className="text-sm text-amber-900">
          ...
          pré-marcados porque o nome parece indicar algo pendente — revise antes de salvar.
        </p>
            ...
            {linhasVisiveis.length === 0 ? (
              ...
              return (
                <tr key={node.id} className={item?.estimado ? 'bg-amber-50/40' : undefined}>
                  ...
                  <td className="px-3 py-2 text-right tabular-nums text-gray-700">{fmt(node.total)}</td>

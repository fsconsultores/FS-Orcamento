...
export function CadernoView({ data }: { data: CadernoData }) {
  ...
  const sections = [
    ...
    { title: 'Itens Estimados', desc: `${data.itensEstimados.reduce((s, g) => s + g.itens.length, 0)} item(ns) marcado(s) como estimado em ${data.itensEstimados.length} planilha(s)`, ok: data.itensEstimados.length > 0 },

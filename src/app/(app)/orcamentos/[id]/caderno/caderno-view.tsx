...
export function CadernoView({ data }: { data: CadernoData }) {
  ...
  const sections = [
    { title: 'Resumo Geral do Orçamento', desc: `Total Orçado (A): ${fmt(totalOrcadoA)} • Serviços Estimados (B): ${fmt(totalServicosEstimadosB)} • Total Geral (A+B): ${fmt(totalOrcadoA + totalServicosEstimadosB)}`, ok: data.arvore.length > 0 },
      ...
      <div className="rounded-lg border border-gray-200 bg-white p-4 flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-0.5 text-sm text-gray-700">
            <p>Total Orçado (A): <span className="font-semibold">{fmt(totalOrcadoA)}</span></p>
            <p>Serviços Estimados (B): <span className="font-semibold">{fmt(totalServicosEstimadosB)}</span></p>
            <p>Total Geral (A+B): <span className="font-semibold">{fmt(totalOrcadoA + totalServicosEstimadosB)}</span></p>
          </div>
        </div>
        <button

...
export function FavoriteButton({
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    startTransition(async () => {
      const result = await toggleFavorito(entityType, entityId)
      if ('error' in result) {
        setFavorito(!next)
  ...
  return (
    <button
      ...
      className={`inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-gray-300 transition-colors hover:bg-amber-50 hover:text-amber-400 ${favorito ? '!text-amber-400' : ''} ${className}`}
    >

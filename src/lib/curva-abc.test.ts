...
describe('Curva ABC é sempre sobre o preço de CUSTO, sem BDI', () => {
  // Pedido explícito do usuário: a Curva ABC não pode ser inflada pelo BDI
  // (margem/lucro) — precisa refletir só o custo (quantidade × custo_unitario),
  // mesmo em orçamentos com bdi_especifico por item. Antes desta mudança, a
  // Curva ABC propositalmente reproduzia "(A) Total Orçado" do Caderno (COM
  // BDI); essa decisão foi revertida a pedido do usuário.
  ...
  const itens = [
    { codigo: null, descricao: 'Betoneira - Locação', unidade: 'MÊS', quantidade: 2, custo_unitario: 100 },
    { codigo: 'ABCTC01', descricao: 'Alvenaria de Vedação', unidade: 'M2', quantidade: 10, custo_unitario: 50 },
  ]

  it('o total da Curva ABC Geral é a soma bruta de custo (200 + 500 = 700), sem nenhum acréscimo de BDI', () => {
  ...
  it('decompõe a composição em insumos preservando quantidade/custo_unitario e valor de custo reais, sem BDI', () => {
    const curva = computeAbcCurvaUnica(itens, composicoes, allInsumos, [])
    ...
    // 10×30=300 e 10×20=200 — puro custo, sem fator de BDI multiplicando.
    expect(insumoA.valor_total).toBeCloseTo(300)
    expect(insumoB.valor_total).toBeCloseTo(200)
  })

  it('abcServicos (não decomposto) soma o mesmo valor de custo que a decomposição em insumos', () => {
    const { abcServicos } = computeAbcCurves(itens, composicoes, allInsumos, [])
    expect(abcServicos).toHaveLength(1)
    expect(abcServicos[0].codigo).toBe('ABCTC01')
    // 10 × 50 = 500 — mesmo valor que Insumo A (300) + Insumo B (200) somados.
    expect(abcServicos[0].valor_total).toBeCloseTo(500)
  })

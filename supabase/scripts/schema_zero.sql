...
-- Script único, executar em projeto Supabase NOVO/LIMPO.
-- =====================================================================
...
-- 1.1) Insumos (materiais, mão de obra, equipamentos)
...
-- 1.2) Composições (serviços formados por insumos)
...
-- 1.3) Itens de composição (relação composição × insumo + índice de consumo)
...
-- Biblioteca compartilhada: leitura aberta para autenticados.
-- Escrita só via service_role (admin/seed).
...
-- Orçamentos: só o dono enxerga e mexe.
CREATE POLICY tabela_orcamentos_owner ON tabela_orcamentos
    FOR ALL TO authenticated
    USING      (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

-- Itens de orçamento: acesso herdado do orçamento-pai.
...
-- =====================================================================
-- 3) CÁLCULO DE CUSTO (views e função auxiliar)
-- =====================================================================

-- 3.1) Função: custo unitário de uma composição
...
-- 3.2) View: custo unitário de cada composição (em tempo real)
...
-- 3.3) View: total de cada orçamento (subtotal e total com BDI)
...
SELECT
    ...
    COALESCE(SUM(
        io.quantidade
        * vc.custo_unitario
        * (1 + COALESCE(io.bdi_especifico, o.bdi_global))
    ), 0)::NUMERIC(14,4) AS total_com_bdi
